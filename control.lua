		...
		if beltEntitiesToReturn[cBUnitNumber] == nil then
			...
			if forceDisregardTier then
				...
				if conBelt.type == "splitter" then
					beltEntitiesToReturn =
						findDownstreamNetwork(conBelt, beltEntitiesToReturn, relUGBeltName, truthTable)
				end
	...
	if thisPlayer.connected and thisPlayer.selected and thisPlayer.controller_type ~= defines.controllers.ghost then
		...
		for beltKey, belt in pairs(transportBeltEntitiesToUpgrade) do
			...
			if belt.prototype.next_upgrade ~= nil then
				belt.order_upgrade({
					target = belt.prototype.next_upgrade,
					force = thisPlayer.force_index,
					player = thisPlayer,
					item_index = 1,
				})
			end
...
-- Used to obtain belt metadata encoded in prototypes. Workaround for https://forums.factorio.com/viewtopic.php?t=126686
local function get_data()
	local s = ""

	local key_base = "belt-upgrades-data"
	local block = 1

	while prototypes.virtual_signal[key_base .. block] do
		s = s .. prototypes.virtual_signal[key_base .. block].order
		block = block + 1
	end
