...
## Belt Thread Remover
Transitioning from the starter base to the mid-game base often leaves long belts that need to be removed between others, which can be tricky and time-consuming to remove without errors. This tool allows the user to select one or more belt pieces, and all belts that would be made redundant by removing the selected belts will be marked for deconstruction. An alternate mode allows all connected belts to be marked for deconstruction, regardless of uses. From version 1.4.0, this removal tool can also be used for pipe networks.
...
### Known issues with some mods
 ...
 - [Loader Utilities](https://mods.factorio.com/mod/loader-utils) adds various options to loaders, and each configuration of these loaders is a separate, hidden entity. Due to the [game engine limitation that requires items to not be hidden](https://forums.factorio.com/viewtopic.php?p=686208#p686208) to upgrade them, only the base loader (with no options selected) works with this mod.
...
**When in cursor:**
  ...
  - Reverse select (by default, right mouse) to remove pipe-like entities connected to the selected pipes, up to the first pipe-like entity that is connected to 3 or more other pipe-like entities, i.e., the first junction. Does not remove crafting buildings.
  - Alternate reverse select (by default, Shift+right mouse) to remove all pipe-like entities connected to the selected pipes. Does not remove crafting buildings.
